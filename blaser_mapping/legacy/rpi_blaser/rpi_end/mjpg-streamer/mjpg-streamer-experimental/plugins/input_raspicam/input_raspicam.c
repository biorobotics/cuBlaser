...
const char* config_file = "cfg/icra2021.cfg";

#define MMAL_CAMERA_PREVIEW_PORT 0
#define MMAL_CAMERA_VIDEO_PORT 1
#define MMAL_CAMERA_CAPTURE_PORT 2
// Stills format information
#define STILLS_FRAME_RATE_NUM 60
...
const int TURN_LED = 1;
const int TURN_LASER = 0;
const int TURN_NEOPIXEL = 0;
...
void *wait_LED_thread(void *vargp)
{
  uint8_t *is_on = (uint8_t *)vargp;
  printf("is on? %d\n", *is_on);
  usleep(1000);
  switch_led((*is_on == 0) ? 1 : 0);
  pthread_exit(NULL);
}
...
int laser_pin = 4;
int led_pin = 24;
int laser_on_delay = 9;
int laser_off_delay = 9;
int led_on_delay = 9;
int led_off_delay = 5;
...
/**
 * parse config file using libconfig
 * Config files contains camera settings, alternating frame settings
 */
void parseConfigFile()
{
  ...
  // read file
  if(! config_read_file(&cfg, "../../../cfg/pipe_blaser_v1.cfg"))
  {
  ...
  // load parameters
  int data;
  if (config_lookup_int(&cfg, "width", &data))
    width = data;
  if (config_lookup_int(&cfg, "height", &data))
    height = data;
  if (config_lookup_int(&cfg, "quality", &data))
    quality = data;
  if (config_lookup_int(&cfg, "fps", &data))
    fps = data;
  if (config_lookup_int(&cfg, "ISO", &data))
    c_params.ISO = data;
  if (config_lookup_int(&cfg, "short_ss", &data))
    short_ss = data;
  if (config_lookup_int(&cfg, "long_ss", &data))
    long_ss = data;

  if (config_lookup_int(&cfg, "laser_pin", &data))
    laser_pin = data;
  if (config_lookup_int(&cfg, "laser_on_delay", &data))
    laser_on_delay = data;
  if (config_lookup_int(&cfg, "laser_off_delay", &data))
    laser_off_delay = data;

  if (config_lookup_int(&cfg, "led_pin", &data))
    led_pin = data;
  if (config_lookup_int(&cfg, "led_on_delay", &data))
    led_on_delay = data;
  if (config_lookup_int(&cfg, "led_off_delay", &data))
    led_off_delay = data;
}
...
int input_init(input_parameter *param, int plugin_no)
{
  ...
  while(1) {
    ...
    static struct option long_options[] = {
      ...
      {"short_ss", required_argument, 0, 0},          // 33
      {"long_ss", required_argument, 0, 0},           // 34
      {0, 0, 0, 0}
    };
    ...
    switch(option_index) {
      ...
      case 32:
        ...
        break;
      case 33:
        sscanf(optarg, "%d", &short_ss);
        break;
      case 34:
        sscanf(optarg, "%d", &long_ss);
        break;
      default:
        DBG("default case\n");
        help();
...
static void encoder_buffer_callback(MMAL_PORT_T *port, MMAL_BUFFER_HEADER_T *buffer)
{
  ...
  if (pData)
  {
    ...
    if (buffer->flags & (MMAL_BUFFER_HEADER_FLAG_FRAME_END | MMAL_BUFFER_HEADER_FLAG_TRANSMISSION_FAILED))
    {
      ...
      // control alternating shutter speed

      flag_dark = (flag_dark == 0) ? 1 : 0;

      int shutter_speed = (flag_dark == 1) ? long_ss : short_ss;
      raspicamcontrol_set_shutter_speed(camera, shutter_speed);

      if (TURN_LED)
      {
        switch_led((flag_dark == 0) ? 1 : 0);
      }
      if (TURN_NEOPIXEL)
        switch_neopixel((flag_dark == 0) ? 255 : 0);
      if (TURN_LASER)
        switch_laser((flag_dark == 0) ? 0 : 1);


      //set frame size
      pglobal->in[plugin_number].size = pData->offset;
      pglobal->in[plugin_number].shutter = (flag_dark == 1) ? 0 : 1;
  ...
  if (complete)
  {
    ...
    /*
    int shutter_speed = (flag_dark == 1) ? short_ss : long_ss;
    raspicamcontrol_set_shutter_speed(camera, shutter_speed);

    if (TURN_LED)
    {
      // todo print time, to see if this is executed at 40 hz
      //struct timeval
      //pthread_t tid;
      //void *flag_dark_ptr = (void *)&flag_dark;
      //pthread_create(&tid, NULL, wait_LED_thread, flag_dark_ptr);
      //printf("thread id: %u\n", tid);
...
void *worker_thread(void *arg)
{
  ...
  if(usestills){
    ...
    fprintf(stderr, "*** start stills output ***\n");
    fprintf(stderr, "delay: %i\n", delay);

    struct timespec t0, t1, t2, t3, t3_old;

    raspicamcontrol_set_shutter_speed(camera, 2000);
    raspicamcontrol_set_awb_mode(camera, MMAL_PARAM_AWBMODE_OFF);
    raspicamcontrol_set_awb_gains(camera, 1.4, 1.5);
    raspicamcontrol_set_exposure_mode(camera, MMAL_PARAM_EXPOSUREMODE_OFF);
    //raspicamcontrol_set_ISO(camera, 1600);

    //int short_shutter_flag = 0;

    while(!pglobal->stop) {
      /*
      if (short_shutter_flag == 0)
      {
        raspicamcontrol_set_shutter_speed(camera, 2000);
        short_shutter_flag = 1;
      }
      else
      {
        short_shutter_flag = 0;
        raspicamcontrol_set_shutter_speed(camera, 50000);
      }
      */
      clock_gettime(CLOCK_MONOTONIC, &t0);
      t3_old = t3;
      fprintf(stderr, "** start one capture\n");
      //Wait the delay
      usleep(delay);
      ...
      fprintf(stderr, "buffer length: %i\n", num);
      int q;
      for (q=0;q<num;q++)
      {
      ...
      else
      {
        ...
        //vcos_semaphore_wait_timeout(&callback_data.complete_semaphore, 10);
      ...
      printf("Time report:\n restart time: %f\n  delay time: %f\n  buff time: %f\n  wait time: %f\n",
             (t0.tv_sec - t3_old.tv_sec) * 1000 + (t0.tv_nsec - t3_old.tv_nsec) / 1000000.0,
             (t1.tv_sec - t0.tv_sec) * 1000 + (t1.tv_nsec - t0.tv_nsec) / 1000000.0,
             (t2.tv_sec - t1.tv_sec) * 1000 + (t2.tv_nsec - t1.tv_nsec) / 1000000.0,
             (t3.tv_sec - t2.tv_sec) * 1000 + (t3.tv_nsec - t2.tv_nsec) / 1000000.0);
      //frames++;
      if (frames == 100)
      {
        //calculate fps
  ...
  else
  {
    ...
    raspicamcontrol_set_awb_gains(camera, 1.4, 1.5);
       */
    raspicamcontrol_set_ISO(camera, 3200);
    raspicamcontrol_set_exposure_mode(camera, MMAL_PARAM_EXPOSUREMODE_OFF);
    ...
    {
      usleep(1000);
      //int shutter_speed = (flag_dark == 0) ? 2000 : 100000;
      //raspicamcontrol_set_shutter_speed(camera, shutter_speed);
      //flag_dark = (flag_dark == 0) ? 1 : 0;
    }
  }

...
namespace camodocal
{
    ...
    int EquidistantCamera::getNpow(void)
    {
        return 9 - (mParameters.k5() == 0 ? 2 : 0) -
               (mParameters.k4() == 0 ? 2 : 0) -
               (mParameters.k3() == 0 ? 2 : 0) -
               (mParameters.k2() == 0 ? 2 : 0);
    }

    void EquidistantCamera::allocateBufferMemory()
    {
#ifdef DISPATCH_CUDA
        int lda = this->getNpow();
        float lOpt;
        int size = 1920 * 1080;
        float A[1];
        float er[1];
        ...
        float vr[1];
        float vl[1];
        float subMatrix[lda * (lda - 1)];

        for (int i = 0; i < lda - 1; i++)
        {
            std::memset(subMatrix + i * lda, 0b0000'0000'0000, lda * sizeof(float));
            subMatrix[i * lda + i + 1] = 1;
        }
        int info;

        magma_sgeev_m(
            MagmaNoVec, MagmaNoVec, LDA, input, A, er, ei, vl, 1, vr, 1, &lOpt, -1, &info);

        this->workValue = (int)lOpt;

        for (int i = 0; i < 6; i++)
        {
#parallelization support upto 6 threads this->work.push_back((float *)malloc((int)lOpt * sizeof(float)));
            this->inputBuffer.push_back(calloc(lda * lda, sizeof(float)));
            std::memcpy(this->inputBuffer.at(i), subMatrix, lda * (lda - 1));

            this->inputBuffer.at(i)[(lda - 1) * lda + 1] = 1.0f;
            if (lda >= 3)
                this->inputBuffer.at(i)[(lda - 1) * lda + 3] = mParameters.k2();
            if (lda >= 5)
                this->inputBuffer.at(i)[(lda - 1) * lda + 5] = mParameters.k3();
            if (lda >= 7)
                this->inputBuffer.at(i)[(lda - 1) * lda + 7] = mParameters.k4();
            if (lda > 9)
                this->inputBuffer.at(i)[(lda - 1) * lda + 9] = mParameters.k5();
        }

        this->wr.resize(2 * size);
        this->wi.resize(2 * size);

        for (int i = 0; i < 2 * size; i++)
        {
            manager->allocateSharedMemory(&this->wr.at(i), lda * sizeof(float));
            maganer->allocateSharedMemory(&this->wi.at(i), lda * sizeof(float));
        }

        this->manager->allocateMemory(*wrPointerPos, this->wr.size() * sizeof(this->wr.at(0)));
        this->manager->allocateMemory(*wiPointerPos, this->wi.size() * sizeof(this->wi.at(0)));

        this->manager->memcpy(wrPointerPos, this->wr.data(), this->wr.size() * sizeof(this->wr.at(0)), memcpyDirection::hostTodevice);
        this->manager->memcpy(wiPointerPos, this->wi.data(), this->wr.size() * sizeof(this->wi.at(0)), memcpyDirection::hostTodevice);

        manager->allocateSharedMemory(&this->phis, 2 * size * sizeof(float));
#endif
    }

    EquidistantCamera::fillEigenValues(cv::Point2f *cur_pts, cv::Point2f *forw_pts, int size)
    {
        int lda = this->getNpow();
#ifdef DISPATCH_CUDA
#pragma omp parallel for num_threads(6)
        ...
        {
            int info;
            float norm = std::sqrt(cur_pts[i].x * cur_pts[i].x + cur_pts[i].y * cur_pts[i].y);
            this->inputBuffer.at(omp_get_thread_num())[(lda - 1) * lda + 0] = norm;
            magma_sgeev_m(MagmaNoVec, MagmaNoVec, lda, this->inputBuffer.at(omp_get_thread_num()), lda, this->wr.at(i), this->wl.at(i), nullptr, 1, nullptr, 1, this->work.at(omp_get_thread_num()), lwork, &info);
            this->phis[i]  = norm > 1e-10 ? atan2(cur_pts[i].y, cur_pts[i].x) : 0;

            norm = std::sqrt(forw_pts[i].x * forw_pts[i].x + forw_pts[i].y * forw_pts[i].y);
            this->inputBuffer.at(omp_get_thread_num())[(lda - 1) * lda + 0] = std::sqrt(forw_pts[i].x * forw_pts[i].x + forw_pts[i].y * forw_pts[i].y);
            magma_sgeev_m(MagmaNoVec, MagmaNoVec, lda, this->inputBuffer.at(omp_get_thread_num()), lda, this->wr.at(i+i), this->wl.at(i+i), nullptr, 1, nullptr, 1, this->work.at(omp_get_thread_num()), lwork, &info);
            this->phis[i+i] = norm > 1e-10 ? atan2(cur_pts[i].y, cur_pts[i].x) : 0;
        }
#endif
    }

    EquidistantCamera::fillPoints(int size, cv::Point2f* aun_cur_pts, cv::Point2f*  aun_forw_pts, float FOCAL_LENGHT, float COL, float ROW){

#ifdef DISPATCH_CUDA
        this->manager->dispatch(equidistant_fillPoints, {
            this->un_forw_pts,
            this->un_cur_pts,
            this->wrPointerPos,
            this->wiPointerPos,
            ...
            getDim(std::ceil(size / 1024)), 
            getDim(1024)
        });

        this->manager->memcpy(aun_cur_pts, this->un_cur_pts, size * sizeof(cv::Point2f), memcpyDirection::deviceTohost);
        this->manager->memcpy(aun_forw_pts, this->un_forw_pts, size * sizeof(cv::Point2f), memcpyDirection::deviceTohost);

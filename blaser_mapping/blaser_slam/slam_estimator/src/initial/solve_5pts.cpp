...
int recoverPose(InputArray E, InputArray _points1, InputArray _points2,
                OutputArray _R,
                OutputArray _t, double focal, Point2d pp,
                InputOutputArray _mask)
{
  Mat cameraMatrix = (Mat_<double>(3, 3)
      << focal, 0, pp.x, 0, focal, pp.y, 0, 0, 1);
...
bool
MotionEstimator::solveRelativeRT(const vector<pair<Vector3d, Vector3d>> &corres,
                                 Matrix3d &Rotation, Vector3d &Translation)
{
  if (corres.size() >= 15) {
    ...
    cv::Mat mask;
    cv::Mat E = cv::findFundamentalMat(ll, rr, cv::FM_RANSAC, 0.3 / 460, 0.99,
                                       mask);
                                 ...
                                 std::vector<bool> &v_mask)
{
  if (corres.size() >= 15) {
    ...
    // load v_mask
    v_mask.reserve(mask.size[0]);
    for (size_t i = 0; i < mask.size[0]; i++)
      v_mask.push_back(mask.data[i] > 0);
    ...
    for (size_t i = 0; i < corres.size(); i++)
    {
      if (!v_mask[i])
        continue;
      Vector3d pw;
      triangulatePoint(Matrix3d::Identity(), Vector3d::Zero(), Rotation,
          Translation, corres[i].second.head(2), corres[i].first.head(2), pw);
    ...
    for (size_t i = 0; i < corres.size(); i++)
    {
      if (!v_mask[i])
        continue;
      Vector3d pw;
      triangulatePoint(Matrix3d::Identity(), Vector3d::Zero(), Rotation.transpose(),
                       -Rotation.transpose() * Translation, corres[i].first.head(2), corres[i].second.head(2), pw);
      std::cout << pw.transpose() << std::endl;
    }
    cout << "lalalalalalalala *************" << endl;
    for (size_t i = 0; i < corres.size(); i++)
    {
      ...
      Vector3d pw;
      triangulatePoint(Matrix3d::Identity(), Vector3d::Zero(), Rotation,
                       Translation, corres[i].first.head(2), corres[i].second.head(2), pw);
      std::cout << pw.transpose() << std::endl;
    }
    */

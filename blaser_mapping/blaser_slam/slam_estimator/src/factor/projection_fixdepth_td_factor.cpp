#include "projection_fixdepth_td_factor.h"

Eigen::Matrix2d ProjectionFixDepthTdFactor::sqrt_info;
double ProjectionFixDepthTdFactor::sum_t;

ProjectionFixDepthTdFactor::ProjectionFixDepthTdFactor(
    ...
    const double _row_j, const double _depth_i)
...
bool ProjectionFixDepthTdFactor::Evaluate(double const *const *parameters,
                                          double *residuals,
                                          double **jacobians) const
{
  ...
  Eigen::Vector3d pts_camera_i = pts_i_td * depth_i;
  ...
  if (jacobians)
  {
    ...
    if (jacobians[3]) // td
    {
      ...
      jacobian_td =
          reduce * ric.transpose() * Rj.transpose() * Ri * ric * velocity_i *
          depth_i * -1.0 +
  ...
  //std::cout << "Feature on laser residuals: "
  //          << residual(0) << " " << residual(1) << std::endl;

  return true;
}

void ProjectionFixDepthTdFactor::check(double **parameters)
{

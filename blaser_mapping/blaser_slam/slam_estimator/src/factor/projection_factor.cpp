...
ProjectionFactor::ProjectionFactor(const Eigen::Vector3d &_pts_i,
    ...
    : pts_i(_pts_i), pts_j(_pts_j), sqrt_info_or(0.)
{
...
ProjectionFactor::ProjectionFactor(const Eigen::Vector3d &_pts_i,
                                   const Eigen::Vector3d &_pts_j,
                                   double _sqrt_info)
    : pts_i(_pts_i), pts_j(_pts_j), sqrt_info_or(_sqrt_info)
{
#ifdef UNIT_SPHERE_ERROR
  Eigen::Vector3d b1, b2;
  Eigen::Vector3d a = pts_j.normalized();
...
bool
ProjectionFactor::Evaluate(double const *const *parameters, double *residuals,
                           double **jacobians) const
{
  ...
  if (sqrt_info_or > 0)
  {
    residual = Eigen::Matrix2d::Identity() * sqrt_info_or * residual;
  }
  else
...
#endif
    if (sqrt_info_or > 0)
      reduce = Eigen::Matrix2d::Identity() * sqrt_info_or * reduce;
    else
...
#if 1
      jacobian_feature =
          reduce * ric.transpose() * Rj.transpose() * Ri * ric * pts_i * -1.0 /
          (inv_dep_i * inv_dep_i);
#else

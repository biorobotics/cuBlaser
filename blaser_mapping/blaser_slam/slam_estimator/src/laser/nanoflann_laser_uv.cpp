...
nanoflann::KDTreeLaserUV::KDTreeLaserUV(LaserPointCloudConstPtr _p_lpc,
                                                bool _sorted)
: adaptor_(_p_lpc)
, kdtree_(2, adaptor_)
{
...
int nanoflann::KDTreeLaserUV::radiusSearch(const Vector2d &pt,
                                               double radius,
                                               std::vector<int> &k_indices,
                                               std::vector<double> &k_sqr_dist) const
{
  static std::vector<std::pair<int, double> > indices_dist;
  indices_dist.reserve(int(radius * 2));

...
# compute point to plane absolute distance and color map
abs_dist = np.abs(pts @ plane_params[0:3, None] + plane_params[3]) / np.linalg.norm(plane_params[0:3])
print("mean point to plane distance: ", np.mean(abs_dist))
abs_dist[abs_dist > 0.002] = 0.002

# plot points and plane
fig = plt.figure()
ax = fig.add_subplot(111,projection='3d')
ax.plot_surface(pl_xx, pl_yy, pl_z, alpha=0.2, color='red')
im = ax.scatter(pts[:,0], pts[:,1], pts[:,2], s=0.5, alpha=0.5, c=abs_dist[:, 0])

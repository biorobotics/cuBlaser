...
CalibProcessor::CalibProcessor(cv::Ptr<calibrationData> data, captureParameters &capParams) : mCalibData(data), mBoardType(capParams.board), mBoardSize(capParams.boardSize)
{
    mCapuredFrames = 0;
    ...
    mMaxTemplateOffset = std::sqrt(static_cast<float>(mCalibData->imageSize.height * mCalibData->imageSize.height) +
                                   static_cast<float>(mCalibData->imageSize.width * mCalibData->imageSize.width)) /
                         20.0;
...
ShowProcessor::ShowProcessor(cv::Ptr<calibrationData> data, cv::Ptr<calibController> controller, TemplateType board) : mCalibdata(data), mController(controller), mBoardType(board)
{
    mNeedUndistort = true;
    mVisMode = Grid;
    mGridViewScale = 0.5;
...
cv::Mat ShowProcessor::processFrameLaser(const cv::Mat &frame)
{
    cv::Mat frameCopy;
    int baseLine = 100;
    ...
    displayMessage = cv::format("Calibrating Laser");
    cv::Size textSize = cv::getTextSize("Calibrating Laser", 1, mTextSize, 2, &baseLine);
...
cv::Mat ShowProcessor::processFrame(const cv::Mat &frame)
{
    ...
    if (mVisMode == VizLaser)
    {
        return processFrameLaser(frame);
    }
    if (!mCalibdata->cameraMatrix.empty() && !mCalibdata->distCoeffs.empty())
    {
        ...
        if (mNeedUndistort && mController->getFramesNumberState())
        {
            ...
            cv::putText(frameCopy, "Undistorted view", textOrigin, 1, mTextSize, textColor, thickness, cv::LINE_AA);
        }
        else
        {
            frame.copyTo(frameCopy);
        ...
        cv::Point textOrigin = cv::Point(baseLine, 2 * textSize.height);
        cv::putText(frameCopy, displayMessage, textOrigin, 1, mTextSize - 1, textColor, thickness, cv::LINE_AA);
        ...
        if (mController->getConfidenceIntrervalsState() && mController->getFramesNumberState())
            displayMessage.append(" OK");
        cv::putText(frameCopy, displayMessage, cv::Point(baseLine, 4 * textSize.height), 1, mTextSize - 1, textColor, thickness, cv::LINE_AA);

        displayMessage = cv::format("Coverage = %.1f\%", mCalibdata->coverageRatio * 100);
        cv::putText(frameCopy, displayMessage, cv::Point(baseLine, 6 * textSize.height), 1, mTextSize - 1, textColor, thickness, cv::LINE_AA);
        
        if (mController->getCommonCalibrationState())
        {
            displayMessage = cv::format("Calibration is done");
            cv::putText(frameCopy, displayMessage, cv::Point(baseLine, 8 * textSize.height), 1, mTextSize - 1, textColor, thickness, cv::LINE_AA);
        }
        ...
        cv::putText(frameCopy, displayMessage, cv::Point(baseLine, frameCopy.rows - (int)(1.5 * textSize.height)),
                    1, mTextSize - 1, textColor, thickness, cv::LINE_AA);
        return frameCopy;
    }
...
void ShowProcessor::switchVisualizationMode()
{
    ...
    {
        std::cout << "Cannot switch mode in laser calib mode" << std::endl;
    }
}
...
LaserCalibProcessor::LaserCalibProcessor(cv::Ptr<calibrationData> data, captureParameters &capParams) :
    mExtractor(data->imageSize.width, data->imageSize.height),
    CalibProcessor(data, capParams)
{
    mExtractor.setLaserMaskingRange(
        cv::Scalar(0, 110, 30),
        cv::Scalar(10, 255, 255),
        cv::Scalar(170, 110, 30),
        cv::Scalar(180, 255, 255)
    );
}

cv::Mat LaserCalibProcessor::processFrame(const cv::Mat &frame)
{
    cv::Mat frameCopy;
    frame.copyTo(frameCopy);
    bool isTemplateFound = false;
    ...
    // Detect laser
    std::vector<cv::Point2f> lpts2d = mExtractor.extract(frame);
    if (isTemplateFound)
    {
        mAllLaserPoints2D.reserve(mAllLaserPoints2D.size() + lpts2d.size());
        mAllLaserPoints2D.insert(mAllLaserPoints2D.end(), lpts2d.begin(), lpts2d.end());
    }
    for (int i = 0; i < mAllLaserPoints2D.size(); i++)
    {
        cv::drawMarker(frameCopy, mAllLaserPoints2D[i], cv::Scalar(0, 255, 0), cv::MARKER_CROSS, 1);
    }

    return frameCopy;
}

bool LaserCalibProcessor::isProcessed() const
{
    return false;
}

void LaserCalibProcessor::resetState()
{
    return;
}

LaserCalibProcessor::~LaserCalibProcessor()
{

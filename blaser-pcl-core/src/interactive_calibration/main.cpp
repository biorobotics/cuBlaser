...
int main(int argc, char **argv)
{
    ...
    if (!parser.has("v"))
        globalData->imageSize = capParams.cameraResolution;

    int calibrationFlags = 0;
    if (intParams.fastSolving)
        calibrationFlags |= cv::CALIB_USE_QR;
    ...
    cv::Ptr<calibDataController> dataController(new calibDataController(globalData, capParams.maxFramesNum,
                                                                        intParams.filterAlpha, intParams.useFishEye));
    dataController->setParametersFileName(parser.get<std::string>("of"));

    cv::Ptr<FrameProcessor> capProcessor, showProcessor, laserProcessor;
    ...
    laserProcessor = cv::Ptr<FrameProcessor>(new LaserCalibProcessor(globalData, capParams));
    ...
    try
    {
        ...
        while (!pipelineFinished)
        {
            ...
            else if (exitStatus == Calibrate)
            {
                ...
                if (capParams.board != chAruco)
                {
                    ...
                    else
                    {
                        cv::Mat p1, p2, p3, p4;
                        cv::calibrateCamera(globalData->objectPoints, globalData->imagePoints,
                                            globalData->imageSize, p1,
                                            p2, cv::noArray(), cv::noArray(),
                                            globalData->stdDeviations, cv::noArray(), globalData->perViewErrors,
                                            calibrationFlags, solverTermCrit);
                        globalData->totalAvgErr =
                ...
                std::cout << controller->getFramesNumberState() << std::endl;
                std::cout << controller->getConfidenceIntrervalsState() << std::endl;
                std::cout << controller->getRMSState() << std::endl;
                std::cout << controller->estimateCoverageQuality() << std::endl;
                std::cout << "---------------" << std::endl;
            }
            ...
            else if (exitStatus == CalibrateLaser)
            {
                std::cout << "[Calibrate Laser]" << std::endl;
                if (!controller->getCommonCalibrationState() && false)
                {
                    std::cout << "Camera calibration is not ready yet" << std::endl;
                }
                else
                {
                    // Swap CalibProcessor with LaserCalibProcessor
                    std::cout << "Removing calib processor" << std::endl;
                    processors.erase(processors.begin());
                    processors.insert(processors.begin(), laserProcessor);
                    static_cast<ShowProcessor *>(showProcessor.get())->setVisualizationMode(calib::VizLaser);
                    std::cout << "Added laser processor" << std::endl;
                }
            }

...
calib::calibController::calibController(cv::Ptr<calib::calibrationData> data, int initialFlags, bool autoTuning, int minFramesNum) : mCalibData(data)
{
...
void calib::calibController::updateState()
{
    ...
    {
        double quality = estimateCoverageQuality();
        mCalibData->coverageRatio = quality / 1.75;
        std::cout << mCalibData->coverageRatio << std::endl;
        mCoverageQualityState = mCalibData->coverageRatio >= 1.f ? true : false;
    }
    else
    {
        // Bad coverage when less than required images
        mCalibData->coverageRatio = 0;
    }
...
{
    return mCalibData->totalAvgErr < 0.68; // TODO: 0.5 for fisheye
}
...
calib::calibDataController::calibDataController(
    cv::Ptr<calib::calibrationData> data, int maxFrames, double convParameter, bool useFishEye) :
    mCalibData(data), mParamsFileName("CamParams.xml"), mUseFishEye(useFishEye)
{
...
void calib::calibDataController::printParametersToConsole(std::ostream &output) const
{
    ...
    {
        output << "K = " << mCalibData->distCoeffs;
    }
}

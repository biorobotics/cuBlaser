...
namespace calib
{
    ...
    public:
        ...
        ~CalibProcessor() override;
    };

    enum visualisationMode
    {
        Grid,    // [Calib camera]: visualize coverage as points in same view
        Window,  // [Calib camera]: Visualize coverage in separate view
        VizLaser // [Calib laser]: Visualize laser plane points in 3D
    };
    ...
    protected:
        ...
        cv::Mat processFrameLaser(const cv::Mat &frame);
    ...
    public:
        ...
        ~ShowProcessor() override;
    };

    class LaserCalibProcessor : public CalibProcessor
    {
    protected:
        int mNeededFramesNum;
        GrayGravityLaserExtractor mExtractor;
        std::vector<cv::Point2f> mAllLaserPoints2D;

    public:
        LaserCalibProcessor(cv::Ptr<calibrationData> data, captureParameters &capParams);
        virtual cv::Mat processFrame(const cv::Mat &frame) override;
        virtual bool isProcessed() const override;
        virtual void resetState() override;
        ~LaserCalibProcessor() override;
    };

...
bool calib::parametersController::loadFromFile(const std::string &inputFileName)
{
    ...
    if (!reader.isOpened())
    {
        std::cerr << "Warning: Unable to open " << inputFileName << " Application started with default advanced parameters" << std::endl;
        return true;
    }
    ...
    readFromNode(reader["camera_roi_corner"], mCapParams.cameraROICorner);
    ...
    readFromNode(reader["use_fish_eye"], mInternalParameters.useFishEye);
...
bool calib::parametersController::loadFromParser(cv::CommandLineParser &parser)
{
    ...
    else
    {
        ...
        if (mCapParams.camID < 0)
        {
            mCapParams.source = XIMEA;
        }
    }
